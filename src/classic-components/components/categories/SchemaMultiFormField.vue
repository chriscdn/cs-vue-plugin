<template>
    <div>
        ...
        <div v-for="(v, i) in valueLocal" :key="i" class="d-flex my-1">
            ...
            <div v-if="isEditable" class="d-flex flex-shrink-0">
                <input v-if="canAdd" type="image" :src="`${$img}add-row.gif`" class="ml-1" width="16px" height="16px" @click="add(i)" />
                <input v-if="canRemove" type="image" :src="`${$img}delete-row.gif`" class="ml-1" width="16px" height="16px" @click="remove(i)" />
            </div>
        </div>
        <!-- </transition-group> -->
