    ...
    >
      ...
      <ul class="rhfacetlist">
        <template v-for="value in facet.displayValues">
          <li
            v-if="isSelectedFacet(facet.queryParameter, value.Facet)"
            :key="`a-${value.Facet}`"
          >
            <strong>{{ value.DisplayValue }}</strong>
            &nbsp;<span class="facetCount">({{ value.Total }})</span> [<a
              href="#"
              @click.prevent="deselectFacet(facet.queryParameter, value.Facet)"
              >x</a
            >]
          </li>
          <li v-else :key="`b-${value.Facet}`">
            <a
              href="#"
              @click.prevent="selectFacet(facet.queryParameter, value.Facet)"
