<template>
	<v-snackbar v-model="show" :color="snackbar.color" :timeout="snackbar.timeout" top center>{{ snackbar.text }}
		<v-btn dark icon @click.native="show = false">
			<v-icon>mdi-close</v-icon> <!-- this will depend on which icons are used -->
		</v-btn>
	</v-snackbar>
</template>
...
export default {
	...
	beforeCreate() {
		...
		Vue.prototype.$snackbarerror = (text, options) => this.queue(text, {
			color: 'error',
			...options
		})
	}
